(()=>{document.addEventListener('DOMContentLoaded',()=>{const decodeHtmlspecialChars=(text)=>{const map={'&amp;':'&','&#038;':'&','&lt;':'<','&gt;':'>','&quot;':'"','&#039;':"'",'&#8217;':'’','&#8216;':'‘','&#8211;':'–','&#8212;':'—','&#8230;':'…','&#8221;':'”',};return text.replace(/\&[\w\d\#]{2,5}\;/g,(m)=>{const n=map[m];return n})};const compare=(original,changed)=>{const display=changed.nextElementSibling;let color='';let pre=null;const diff=JsDiff.diffLines(original.innerHTML,changed.innerHTML);const fragment=document.createDocumentFragment();diff.forEach((part)=>{if(part.added){color='#a6f3a6'}else if(part.removed){color='#f8cbcb'}else{color=''}
...
const diffState={class:e.target.className,title:e.target.innerHTML,display:diffMain.style.display,};if(typeof(Storage)!=='undefined'){localStorage.setItem('diffButtonState',JSON.stringify(diffState))}}}};const buttons=[].slice.call(document.querySelectorAll(`[${buttonDataSelector}]`));const conditionalSection=document.getElementById('conditional-section');const setPrestate=()=>{if(typeof(Storage)!=='undefined'){const cState=JSON.parse(localStorage.getItem('coreButtonState'));const dState=JSON.parse(localStorage.getItem('diffButtonState'));if(cState!==null&&corePane&&override){buttons[0].className=cState.class;buttons[0].innerHTML=cState.title;corePane.style.display=cState.display;override.className=cState.overrideClass}
...
const diffs=[].slice.call(document.querySelectorAll('#original'));for(let i=0,l=diffs.length;i<l;i+=1){compare(diffs[i],diffs[i].nextElementSibling)}})})()